...
public class BaseCommandParser implements CommandParser {
	...
	private void runOneCommand() {
		...
		if (tokens.size() == 0){
			...
			game.messageOut.println("Error: too many tokens entered!");
			dataRecieved = false;
		}
		...
		if (dataRecieved) {
			...
			if(wordList1.size() == 0){
				game.messageOut.println("Error: no Words matched to token1 ("+token1+")");
				dataRecieved = false;
			}
			else if(wordList1.size() >=2){
				game.messageOut.println("Error: multiple Word matches with token1 ("+token1+")");
				dataRecieved = false;
			}
			else
			...
			if(dataRecieved){
				if(tokens.size()==2){
					...
					if(token2!=null && wordList2.size() == 0){
						game.messageOut.println("Error: no Words matched to token2 ("+token2+")");
						game.messageOut.flush();
						dataRecieved = false;
					}
					else if(wordList2.size() >1){
						game.messageOut.println("Error: multiple Word matches with token2 ("+token2+")");
						game.messageOut.flush();
						dataRecieved = false;
					}
					else
		...
		if(dataRecieved){
			...
			if(act ==null)
				game.messageOut.println("Error: Act is null");
			...
			if (act != null) {
				...
				game.messageOut.print("Error: no Action matches command entered");
	...
	public ArrayList<Word> matchTokenToWord(String token) {
		...
		for (Word wrd : game.allWords) {
			if(wrd.getWord().equals(token)){
				ans.clear();
				ans.add(wrd);
			}else if(wrd.match(token) == MatchType.PREFIX && wrd.getWord().startsWith(token)){
				ans.add(wrd);
			}
...
//			}
    ...
    public ArrayList<String> promptUser (Scanner sc){
		...
		game.messageOut.println("Enter a command: ");
		game.messageOut.flush();
		...
		while (st.hasMoreTokens()) {
			...
			for(Word w : game.noisewords.get(0).getWords()){
