...
public class HardCodedGame implements GameDescription {
	...
	public Game build(InputStream in, PrintStream out) {
		...
		builder.makeAction(vGet, vObjects, new ActionMethod() {
			...
			public void doAction(Game game, Word w1, Word w2) {
				...
				if (game.thePlayer.contains(obj)) {
				    game.messageOut.printf("You're already carrying %s.", obj.getInventoryDesc());
				} else if (game.thePlayer.getLocation().contains(obj)) {
				    game.thePlayer.getLocation().removeObject(obj);
					game.thePlayer.addObject(obj);
					game.messageOut.printf("You're now carrying %s.", obj.getInventoryDesc());
				} else {
				    game.messageOut.printf("I can't find %s here.", obj.getInventoryDesc());
				}
			}
		});
		...
		builder.makeAction(vDrop, vObjects, new ActionMethod() {
			...
			public void doAction(Game game, Word w1, Word w2) {
				...
				if (game.thePlayer.contains(obj)) {
					...
					game.messageOut.printf("You've dropped %s.", obj.getInventoryDesc());
				} else {
				    game.messageOut.printf("You're not carrying %s.", obj.getInventoryDesc());
				}
			}
		});
		...
		builder.makeAction(vExamine, vObjects, new ActionMethod() {
			...
			public void doAction(Game game, Word w1, Word w2) {
				...
				if (game.thePlayer.contains(obj)) {
				    ...
				    game.messageOut.printf("You are not carrying %s.", obj.getInventoryDesc());
				}
			}
		});
		...
		builder.makeAction(vInventory, null, new ActionMethod() {
			...
			public void doAction(Game game, Word w1, Word w2) {
				...
				if (c.isEmpty()) {
				    ...
				    game.messageOut.print("You are carrying ");
					boolean first = true;
					for (GameObject obj : c) {
						...
						game.messageOut.print(obj.getInventoryDesc());
					}
					game.messageOut.print(".");
				}
			}
		...
		builder.makeAction(vRead, vMessage, new ActionMethod() {
		    final GameObject message = iMessage;
			@Override
			public void doAction(Game game, Word w1, Word w2) {
			    if (game.thePlayer.contains(message))
			        game.messageOut.printf("The %s says, \"Enjoy your game.\"", w2.getWord());
			    else
			        game.messageOut.printf("You don't have a %s to read.", w2.getWord());
			}
		});
