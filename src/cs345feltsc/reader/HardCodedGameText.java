 ...
 */
public class HardCodedGameText implements GameDescription {
	...
	public HardCodedGameText(Builder builder) {
	...
	public Game build(InputStream in, PrintStream out) {
		...
		builder.makeAction(vMove, vDirect, new ActionMethod() {
			@Override
			public void doAction(Game game, Word w1, Word w2) {
				game.messageOut.printf("Move to the %s.", w2.getWord());
			}
		});
		...
		builder.makeAction(vLook, null, new ActionMethod() {
			@Override
			public void doAction(Game game, Word w1, Word w2) {
			    game.messageOut.print("I'm looking but I don't see anything.");
			}
		});
		...
		builder.makeAction(vMagic, null, new ActionMethod() {
			@Override
			public void doAction(Game game, Word w1, Word w2) {
				System.out.printf("Nothing happens.");
			}
		});
